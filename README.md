...
0.3 游戏库体验版本新增“扫描 → 审核 → 修正 → 导入 → 整理”连续流程。扫描只读取本地目录、启动文件与可选的 `gameshelf.json`，不会联网；详见 [批量扫描与本地元数据](docs/LIBRARY-IMPORT.md)。0.2 的数据安全说明见 [数据安全、恢复与 Windows 验收](docs/RECOVERY.md)。
