...
- [ ] GitHub Actions 在 1.0 提交上通过 typecheck、测试、生产依赖审计、安装版和便携版构建。
- [ ] Manual：安装版安装、启动和卸载；确认数据位于 `%APPDATA%\GameShelf`。
- [ ] Manual：便携 ZIP 从可写目录启动；确认 `gameshelf-portable` 与旁置 `data` 生效。
- [ ] Manual：用测试数据验证安装版和便携版覆盖升级后数据库、合集、封面和设置保留。
- [ ] Manual：核对应用内版本为 1.0.0，两个产物名称正确，SHA-256 与 `SHA256SUMS.txt` 一致。
